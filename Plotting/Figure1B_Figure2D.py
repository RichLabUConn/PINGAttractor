...
"""
def run_sim(labeltext, inputscale):
    ...
    for q in range(trials):
        ...
        Iext_e_use=np.zeros((numcells_e,1))
        Iext_i_use=np.zeros((numcells_i,1))
        ...
        for i in range(int(T/dt)):
            ...
            Iapp_e=Iext_e_use-np.multiply(Isynin_ee,(v_e[:,-2]-esyn_e))-np.multiply(Isynin_ie,(v_e[:,-2]-esyn_i))
            Iapp_i=Iext_i_use-np.multiply(Isynin_ei,(v_i[:,-2]-esyn_e))-np.multiply(Isynin_ii,(v_i[:,-2]-esyn_i))
        ...
        filenamestr=r'Rasters/FiringRateGauss_Raster_Esyni%1.0f_NoiseAmp%1.0f_%d_start%1.0f_stop%1.0f_%s.png' \
            % (esyn_i, D*1000000, number, timestart, timestop, labeltext)
...
#%% Run 
# for D in [0.0, 0.00002, 0.0006, 0.002]:
for D in [0.0006]:

    for inputscale in [1,2,3,4]:
        if inputscale==2:
            labeltext='HalvedIexte_500ms'
        if inputscale==3:
            labeltext='ThirdIexte_500ms'
        if inputscale==4:
            labeltext='QuarteredIexte_500ms'
        if inputscale==1:
            labeltext='Default'
