...
#%% Loop over all SpikingData csv files

for noiseamp in [0,1,20,70,200,600,1000,2000,4000,6000,8000]:
    ...
    for number in np.arange(1,101,1):
        ...
        # CSV
        str1='CSV/NewBurstFreq_OUNewInitial_Trials%d_NoiseAmp%1.0f_%d.csv' \
             % (1, noiseamp, number)
        ...
        with open(str1, 'a', newline='') as csvfile:
            ...
            for Iext in [25, 28, 30, 33, 35, 38, 40, 43, 45, 48, 50]:    
                print('Iext= ', Iext)
                    
                #%% Data processing
                #Check if file exists, and if so load it 
                
                str1='CSV_Spiking/SpikingData_OUNewInitial_NoiseAmp%1.0f_Iext%1.0f_%d.csv' \
                     % (noiseamp, Iext, number)
