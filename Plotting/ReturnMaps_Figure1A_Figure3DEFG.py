...
#%% Loop over all possible SpikingData csv files

# for noiseamp in [1, 20, 2000, 4000, 8000, 20000, 50000]:
#     print('Noiseamp= ', noiseamp)
#     for number in np.arange(1,26,1):
#         print('Number= ', number)
#         for esyn_i in np.arange(-75, -65+1, 1):    
    
# for noiseamp in [0, 1, 20, 70, 200, 600, 1000, 2000]:
    
# paramsets=[[-70, 0, 2, 712], [-70, 20, 1, 295], [-70, 600, 3, 737], [-70, 2000, 1, 752]]
paramsets=[[-75, 0,1, 760]]
...
for params in paramsets:
    ...
    str1='CSV_Spiking_Figure3/SpikingData_OU_Esyni%1.0f_NoiseAmp%1.0f_%d.csv' \
         % (esyn_i, noiseamp, number)
    ...
    if os.path.exists(str1)==True:
        ...
        for i in range(len(names)):
            ...
            vars()["peaks_"+names[i]], vars()["peak_heights_"+names[i]]= \
                find_peaks(vars()["firingrate_"+names[i]],height=5, prominence=20, distance=75)
        ...
        #%% Setup plotting
        if len(peaktimes_shorter)>1:
            cut=np.argmax(peaktimes_shorter>250)
            peaktimes_shorter=peaktimes_shorter[cut:]
            peakheights_shorter=peakheights_shorter[cut:]
            ...
            gs = fig.add_gridspec(3,2, hspace=.3)
            ...
            #%% Setup Gaussian Return and other labels

            ax1.set_xlabel("Gaussian Height n", fontsize=24)
            ax1.set_ylabel("Gaussian Height n+1", fontsize=24)
               
            
            x=peakheights_shorter[:-1]
            y=peakheights_shorter[1:]
            
            jetmap=cm.get_cmap('jet', len(peakheights_shorter))
            colors=[]
            for i in range(len(peakheights_shorter)):
                colors.append(jetmap(i))
            
            ax1.plot([0,125], [0,125], 'k-', zorder=-1)
            ax1.set_xlim(0,125)
            ax1.set_ylim(0,125)
            
            graph=ax1.scatter([],[], zorder=1)

            #%% Miscellaneous setup
            
            # cell=np.random.randint(0,800)

            ax2.set_xlabel("Time (ms)", fontsize=24)
            ax2.set_ylabel('E cell firing rate', fontsize=24)      

            ax3.set_xlabel("Time (ms)", fontsize=24)
            ax3.set_ylabel("Neuron Index", fontsize=24)
                
            #%% Plot firing rate gaussian                
            window=1000
                               
            legstr1='Gaussian SD: %d ms' % window_shorter
            legstr2='Gaussian SD: %d ms' % window_short
            legstr3='Gaussian SD: %d ms' % window_mid
            legstr4='Gaussian SD: %d ms' % window_long
            ...
            line1, =ax2.plot([peaktimes_shorter[0]-20, peaktimes_shorter[0]-20], 
                            [0, 125],
                            lw=2, color='k', alpha=.5)
            
            line2, =ax2.plot([peaktimes_shorter[0]+20, peaktimes_shorter[0]+20], 
                            [0, 125],
                            lw=2, color='k', alpha=.5)
                   
            #%% Plot Raster
            
            #Get ISIs for given cell, plus other setup
            duration_sec=(T-1250)/1000
            spikes_e_end=processSpikesForSync(spikes_e, 1250, T, numcells_e)
            spikes_e_end_plot=[spikes_e_end[cell][x]+1250 for x in range(len(spikes_e_end[cell]))]

            ISI=[spikes_e_end[cell][x+1]-spikes_e_end[cell][x] for x in range(len(spikes_e_end[cell])-1) ]
        
            times=np.arange(1250, T, 1)
            ...
            for i in range(len(spikes_e)):
                for j in range(len(spikes_e[i])):
                    ...
                    if i==cell:
                        spikes_cell_plot.append([i,spikes_e[i][j]])
            spikes_e_plot_array=np.array(spikes_e_plot)
            spikes_cell_plot_array=np.array(spikes_cell_plot)
            ...
            ax3.scatter(spikes_e_plot_array[:,1], spikes_e_plot_array[:,0], marker=".", s=.1, c='g')
            ax3.scatter(spikes_i_plot_array[:,1], spikes_i_plot_array[:,0], marker=".", s=.1, c='r')
            ax3.scatter(spikes_cell_plot_array[:,1], spikes_cell_plot_array[:,0], marker="*", s=10, c='b')
            line3, =ax3.plot([spikes_e_end_plot[0], spikes_e_end_plot[1]], 
                            [cell, cell],
                            lw=2, color='b')
            ax3.set_ylim(0,1000)
            ...
            ax4.set_xlabel("ISI n (ms)", fontsize=24)
            ax4.set_ylabel("ISI n+1 (ms)", fontsize=24)
                               
            x2=ISI[:-1]
            y2=ISI[1:]
            
            jetmap2=cm.get_cmap('jet', len(ISI))
            colors2=[]
            for i in range(len(ISI)):
                colors2.append(jetmap2(i))
                
                
            
            gaussian_ISI=peaktimes_shorter[10:]-peaktimes_shorter[9:-1]
            burstISI=np.mean(gaussian_ISI)


            
            ax4.plot([0,max(ISI)+10], [0,max(ISI)+10], 'k-', zorder=-1)
            ax4.plot([burstISI, burstISI], [0,burstISI], 'k-', alpha=.5, zorder=-1)
            ax4.plot([0,burstISI], [burstISI, burstISI], 'k-', alpha=.5, zorder=-1)
            ax4.plot([burstISI*2, burstISI*2], [0,burstISI*2], 'k-', alpha=.3, zorder=-1)
            ax4.plot([0,burstISI*2], [burstISI*2, burstISI*2], 'k-', alpha=.3, zorder=-1)
            ax4.plot([burstISI*3, burstISI*3], [0,burstISI*3], 'k-', alpha=.1, zorder=-1)
            ax4.plot([0,burstISI*3], [burstISI*3, burstISI*3], 'k-', alpha=.1, zorder=-1)
            ax4.set_xlim(0,max(ISI)+10)
            ax4.set_ylim(0,max(ISI)+10)
            
            graph2=ax4.scatter([],[], zorder=1)
            ...
            if len(peakheights_shorter)>2:
                ax1.set_box_aspect(1)

                
                ani=FuncAnimation(fig,
                                  partial(animate, graph=graph, ax=ax1, peaks=peakheights_shorter, numpeaks=len(peakheights_shorter),
                                  line1=line1, line2=line2, line3=line3, graph2=graph2,
                                  numISI=len(ISI)),
                                  frames=np.arange(0,len(peakheights_shorter)-1,1), repeat=False, interval=25)         
                 
            
                         
                filenamestr='DualReturnSnapshots_Figure3/DualReturn_FiringRateGauss_Raster_Esyni%1.0f_NoiseAmp%1.0f_%d_Cell%d.mp4' \
                    % (esyn_i, noiseamp, number, cell)
                    
                writervideo=animation.FFMpegWriter(fps=30)
                    
               
                ani.save(filenamestr, writer=writervideo)
            
                ani.event_source.stop()
                del ani
                
                filenamestr2='DualReturnSnapshots_Figure3/DualReturn_FiringRateGauss_Raster_Esyni%1.0f_NoiseAmp%1.0f_%d_Cell%d.png' \
                    % (esyn_i, noiseamp, number, cell)
                plt.savefig(filenamestr2, dpi=600, format='png', bbox_inches='tight')
