...
for rep in np.arange(1, repetitions+1, 1):    
    ...
    print(rep)
    str1='CSV/NewBurstFreq_OUNewInitial_Trials1_NoiseAmp%d_%d.csv' \
         % (noiseamp, rep)
    ...
    # Pad with Nan's
    if int(stats_csv[-1][0])!=Iext_e_max:
        for pad in np.arange(stats_csv[-1][0]+1,Iext_e_max+1,1):
            temp=np.empty((1,22))
            temp[0,0]=pad
            temp[0,1:]=np.nan
    ...
    freq_means[rep-1]=stats_csv[:,1]

freq_means[freq_means==0]=np.nan
...
str1='AverageOverRepetitionsPlots/NewBurstFreq_VaryIext_Repetitions%d_NoiseAmp%d.csv' \
...
with open(str1, 'w', newline='') as csvfile:
    ...
    for i in range(len(Iext_e)):
        mystr=[Iext_e[i], freq_means_mean[i], freq_means_std[i]]
