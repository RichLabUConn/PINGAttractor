...
for noiseamp in [0, 2000]:
    ...
    # CSV
    Iext_e=30
    str1='GammaQuantification73\Stats_GammaFeaturesIFRH_EGABA73_NoiseAmp%1.0f_Iext%1.0f_v2.csv' % (noiseamp, Iext_e)
    ...
    with open(str1, 'a', newline='') as csvfile:
        ...
        for number in np.arange(1,101,1):
            ...
            str1=r'EGABA -73\CSV_Spiking\SpikingData_OUNewInitial_NoiseAmp%1.0f_Iext%1.0f_%d.csv' \
            ...
            if os.path.exists(str1)==True:
                ...
                if np.isnan(syncstart)==False:
                    ...
                    cut_stop=T/1000
                nonPINGduration.append(cut_stop-cut_start)
                ...
                # t_start=np.argmax(t_e>cut_start)+1
                # if cut_stop<20:
                #     t_stop=np.argmin(t_e<cut_stop)-2
                # else:
                #     t_stop=len(t_e)
                    
                # if t_start<t_stop:           
                #     t_e_cut=t_e[t_start:t_stop]
                #     t_i_cut=t_i[t_start:t_stop]
                #     Sxx_e_cut=Sxx_e[:,t_start:t_stop]
                #     Sxx_i_cut=Sxx_i[:,t_start:t_stop]
            
                #     f_start=np.argmax(f_e>30)
                #     f_stop=np.argmin(f_e<80)-1
                #     f_cut=f_e[f_start:f_stop]
                #     Sxx_e_gamma=Sxx_e_cut[f_start:f_stop,:]
                #     Sxx_i_gamma=Sxx_i_cut[f_start:f_stop,:]
                    
                #     summedgamma_e=np.sum(Sxx_e_gamma, axis=0)
                #     summedgamma_i=np.sum(Sxx_i_gamma, axis=0)
                    
                    # #%% Threshold and characterize
                    # gammaevents_index=np.where(summedgamma_e>1)[0]
                    
                    # #Group if gamma persists
                    # gammaevents_index_grouped=[]
                    # track=0
                    # for x in range(len(gammaevents_index)):
                    #     if x<len(gammaevents_index)-1:
                    #         if (gammaevents_index[x+1]-gammaevents_index[x])>1:
                    #             if track==0:
                    #                 gammaevents_index_grouped.append(np.array(gammaevents_index[x]))
                    #                 track=0
                    #             else:
                    #                 gammaevents_index_grouped.append(gammaevents_index[x-track:x+1])
                    #                 track=0
                    #         else:
                    #             track=track+1
                    #     else:
                    ...
                    # for x in gammaevents_index_grouped:
                    #     gamma_duration.append(x.size*(t_e[1]-t_e[0]))
                    #     Sxx_e_gamma_event=Sxx_e_gamma[:,x]
                    #     if x.size>1:
                    #         Sxx_e_gamma_event=np.sum(Sxx_e_gamma_event, axis=1)
                    #     peakfreq_index=np.argmax(Sxx_e_gamma_event)
                    #     gamma_peakfreq.append(f_cut[peakfreq_index])
            
                    # gamma_rate=len(gammaevents_index_grouped)/(t_e_cut[-1]-t_e_cut[0])
                    # nonPINGtime=(t_e_cut[-1]-t_e_cut[0])
                    # gamma_duration_mean=np.mean(gamma_duration)
                    # gamma_peakfreq_mean=np.mean(gamma_peakfreq)
                ...
                #     gamma_rate=0
                #     nonPINGtime=0
                #     gamma_duration_mean=np.nan
                #     gamma_peakfreq_mean=np.nan
                ...
                #Find statistics of long enough gamma groups, then append to csv
                for gamma in gamma_grouped:
                    if len(gamma)>3:
                        t_min=np.argmax(t_e>(peaktimes_shorter_i[gamma[0]]/1000))
                        t_max=np.argmin(t_e<(peaktimes_shorter_i[gamma[-1]]/1000))-1
                        if t_max<=t_min:
                            t_max=t_min+1
                        if (peaktimes_shorter_i[gamma[-1]]/1000)<cut_stop or np.isnan(syncstart)==True:
                            Sxx_e_temp=Sxx_e[:,t_min:t_max+1]
                            gamma_peakfreq_index_temp=np.unravel_index(np.argmax(Sxx_e_temp, axis=None), Sxx_e_temp.shape)
                            gamma_peakfreq_index=gamma_peakfreq_index_temp[0]
                            gamma_peakfreq=f_e[gamma_peakfreq_index]
                            gamma_duration=(peaktimes_shorter_i[gamma[-1]]-peaktimes_shorter_i[gamma[0]])/1000
                            
                            mystr=[number, cut_stop-cut_start, peaktimes_shorter_i[gamma[0]]/1000, peaktimes_shorter_i[gamma[-1]]/1000, gamma_duration, gamma_peakfreq]
                            outputwriter.writerow(mystr)
                            csvfile.flush()

    str2=r'GammaQuantification73\NonPINGDuration_EGABA73_NoiseAmp%1.0f_Iext%1.0f.csv' % (noiseamp, Iext_e)
    with open(str2, 'a', newline='') as csvfile2:
        outputwriter2=csv.writer(csvfile2)
        for j in range(len(nonPINGduration)):
            mystr=[j+1, nonPINGduration[j]]
            outputwriter2.writerow(mystr)
            csvfile2.flush()
