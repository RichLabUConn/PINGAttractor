    ...
    if G_rescaled>.5:  
        ...
        if len(peaks)>2:
            ...
            for i in range(len(peaks)-1):
                if 1 < 1000/(times[peaks[i+1]]-times[peaks[i]]) < 100:
...
# noiseAmps=[noiseAmps[x] for x in [0,1,2,3, 4]]
# extralabel='small'

# noiseAmps=[noiseAmps[x] for x in [5,6,7,8,9]]
# extralabel='large'

noiseAmps=[noiseAmps[x] for x in [0,1,2,3,5,7,8,9]]
extralabel='sample'
...
#%% Intitialize storage
sync_means_mean=np.zeros((esynnum, len(noiseAmps)))
sync_varOverTime_mean=np.zeros((esynnum, len(noiseAmps)))
freq_means_mean=np.zeros((esynnum, len(noiseAmps)))
freq_varOverTime_mean=np.zeros((esynnum, len(noiseAmps)))
MFF_means_mean=np.zeros((esynnum, len(noiseAmps)))
MFF_varOverTime_mean=np.zeros((esynnum, len(noiseAmps)))
percent_means_mean=np.zeros((esynnum, len(noiseAmps)))
percent_varOverTime_mean=np.zeros((esynnum, len(noiseAmps)))

sync_means_std=np.zeros((esynnum, len(noiseAmps)))
sync_varOverTime_std=np.zeros((esynnum, len(noiseAmps)))
freq_means_std=np.zeros((esynnum, len(noiseAmps)))
freq_varOverTime_std=np.zeros((esynnum, len(noiseAmps)))
MFF_means_std=np.zeros((esynnum, len(noiseAmps)))
MFF_varOverTime_std=np.zeros((esynnum, len(noiseAmps)))
percent_means_std=np.zeros((esynnum, len(noiseAmps)))
percent_varOverTime_std=np.zeros((esynnum, len(noiseAmps)))

synconset_mean=np.zeros((esynnum,len(noiseAmps)))
synconset_std=np.zeros((esynnum,len(noiseAmps)))
syncoffset_mean=np.zeros((esynnum,len(noiseAmps)))
syncoffset_std=np.zeros((esynnum,len(noiseAmps)))
syncduration_mean=np.zeros((esynnum,len(noiseAmps)))
syncduration_std=np.zeros((esynnum,len(noiseAmps)))

DCtoSpike_corr_e_mean=np.zeros((esynnum,len(noiseAmps)))
DCtoSpike_corr_i_mean=np.zeros((esynnum,len(noiseAmps)))
DCtoISImean_corr_e_mean=np.zeros((esynnum,len(noiseAmps)))
DCtoISImean_corr_i_mean=np.zeros((esynnum,len(noiseAmps)))
DCtoISIstd_corr_e_mean=np.zeros((esynnum,len(noiseAmps)))
DCtoISIstd_corr_i_mean=np.zeros((esynnum,len(noiseAmps)))
ISImeantoISIstd_corr_e_mean=np.zeros((esynnum,len(noiseAmps)))
ISImeantoISIstd_corr_i_mean=np.zeros((esynnum,len(noiseAmps)))

DCtoSpike_corr_e_std=np.zeros((esynnum,len(noiseAmps)))
DCtoSpike_corr_i_std=np.zeros((esynnum,len(noiseAmps)))
DCtoISImean_corr_e_std=np.zeros((esynnum,len(noiseAmps)))
DCtoISImean_corr_i_std=np.zeros((esynnum,len(noiseAmps)))
DCtoISIstd_corr_e_std=np.zeros((esynnum,len(noiseAmps)))
DCtoISIstd_corr_i_std=np.zeros((esynnum,len(noiseAmps)))
ISImeantoISIstd_corr_e_std=np.zeros((esynnum,len(noiseAmps)))
ISImeantoISIstd_corr_i_std=np.zeros((esynnum,len(noiseAmps)))

linreg_r_mean=np.zeros((esynnum,len(noiseAmps)))
linreg_r_std=np.zeros((esynnum,len(noiseAmps)))
linreg_slope_mean=np.zeros((esynnum,len(noiseAmps)))
linreg_slope_std=np.zeros((esynnum,len(noiseAmps)))

inconsistentsync=np.zeros((esynnum,len(noiseAmps)))
...
for i, noiseamp in enumerate(noiseAmps): 
    ...
    str2='AverageOverRepetitionsPlots/NewBurstFreq_VaryEsynI_Repetitions%d_NoiseAmp%d.csv' \
         % (repetitions, noiseamp)
    ...
    #%% New Processing: Non-consistent synchrony
    for rep in np.arange(1, repetitions+1, 1):    
        ...
        for j in range(len(stats_csv)):
            if stats_csv[j,11]<18750:
                inconsistentsync[j,i]=inconsistentsync[j,i]+1
                
inconsistentsync=inconsistentsync/repetitions
...
ax.plot(esyn_i, inconsistentsync[:,0], lw=3)
ax.set_ylabel('Proportion of trials with IOC')
ax.set_xlabel(r'$E_{GABA}$ (mV)')
ax.spines[['right', 'top']].set_visible(False)

filenamestr=r'PaperFigures/Figure2/InconsistentSync_Noise0.png'
...
ax2.plot(esyn_i, freq_means_mean[:,0], color=colors[0], lw=3, label='Network Oscillation Frequency')
ax2.fill_between(esyn_i, freq_means_mean[:,0]+freq_means_std[:,0],freq_means_mean[:,0]-freq_means_std[:,0], color=colors[0], alpha=.3)

ax2.plot(esyn_i, MFF_means_mean[:,0], color=colors[1], lw=3, label='Neuron Spike Rate')
ax2.fill_between(esyn_i, MFF_means_mean[:,0]+MFF_means_std[:,0],MFF_means_mean[:,0]-MFF_means_std[:,0], color=colors[1], alpha=.3)
...
filenamestr=r'PaperFigures/Figure2/Frequencies_Noise0.png'
plt.savefig(filenamestr, dpi=600, format='png', bbox_inches='tight')
plt.show(block=False)
plt.close()
...
for i, noiseamp in enumerate(noiseAmps):
    ...
    ax.plot(esyn_i, inconsistentsync[:,i], color=cmap(i/(len(noiseAmps)-1)), label=str1, lw=3)
    # ax.fill_between(esyn_i, syncoffset_mean[:,i]+syncoffset_std[:,i],syncoffset_mean[:,i]-syncoffset_std[:,i], 
    #                 color=cmap(i/(len(noiseAmps)-1)), alpha=.3)
# ax.legend(title='Voltage SD', ncol=2)

ax.set_ylabel('Proportion of trials with IOC')
ax.set_xlabel(r'$E_{GABA}$ (mV)', fontsize=24)
# ax.set_xticks(np.arange(-75,esynmax+1,2))
# ax.set_ylim([0, .5])
ax.spines[['right', 'top']].set_visible(False)

filenamestr=r'PaperFigures/Figure3/InconsistentSync_AllNoise.png'
plt.savefig(filenamestr, dpi=600, format='png', bbox_inches='tight')
...
freq_means_mean[:,-1]=np.NaN
freq_means_mean[:,-2]=np.NaN
freq_means_std[:,-1]=np.NaN
freq_means_std[:,-2]=np.NaN

for i, noiseamp in enumerate(noiseAmps):
    str1='%1.3f' % (NoiseAmp_to_VoltageSD[noiseamp])
    ax.plot(esyn_i, freq_means_mean[:,i], color=cmap(i/(len(noiseAmps)-1)), label=str1, lw=3)
    ax.fill_between(esyn_i, freq_means_mean[:,i]+freq_means_std[:,i],freq_means_mean[:,i]-freq_means_std[:,i], 
                    color=cmap(i/(len(noiseAmps)-1)), alpha=.3)
# ax.legend(title='Voltage SD', ncol=2)

ax.set_ylabel('Network Oscillation \n Frequency (Hz)')
ax.set_xlabel(r'$E_{GABA}$ (mV)')
# ax.set_xticks(np.arange(-75,esynmax+1,2))
# ax.set_ylim([0, 1])
ax.spines[['right', 'top']].set_visible(False)

filenamestr=r'PaperFigures/Figure3/BurstFreq_AllNoise.png'
plt.savefig(filenamestr, dpi=600, format='png', bbox_inches='tight')
...
for i, noiseamp in enumerate(noiseAmps):
    str1='%1.3f' % (NoiseAmp_to_VoltageSD[noiseamp])
    ax.plot(esyn_i, MFF_means_mean[:,i], color=cmap(i/(len(noiseAmps)-1)), label=str1, lw=3)
    ax.fill_between(esyn_i, MFF_means_mean[:,i]+MFF_means_std[:,i],MFF_means_mean[:,i]-MFF_means_std[:,i], 
                    color=cmap(i/(len(noiseAmps)-1)), alpha=.3)
ax.legend(title='Voltage SD', ncol=1, bbox_to_anchor=(1.00, 1.0))

ax.set_ylabel('Neuron Spike Rate (Hz)')
ax.set_xlabel(r'$E_{GABA}$ (mV)')
# ax.set_xticks(np.arange(-75,esynmax+1,2))
# ax.set_ylim([0, 1])
ax.spines[['right', 'top']].set_visible(False)

filenamestr=r'PaperFigures/Figure3/SpikeFreq_AllNoise.png'
plt.savefig(filenamestr, dpi=600, format='png', bbox_inches='tight')
