...
for i, noiseamp in enumerate(noiseAmps): 
    ...
    print(noiseamp)
    str1='EGABA -73/AverageOverRepetitionsPlots/VaryIext_Repetitions%d_NoiseAmp%d.csv' \
         % (repetitions, noiseamp)
...
for E_GABA in E_GABA_list:
    for NoiseAmp in NoiseAmp_list:
        ...
        ax.plot(Iext_list, syncpercent_USE[listindex], color=cmap(listindex/(len(syncpercent_USE)-1)), label=str1, lw=2)
