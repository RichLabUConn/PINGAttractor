...
for noiseamp in [0,1,20,70,200,600,1000,2000,4000,6000,8000]:
    ...
    for number in np.arange(1,101,1):
        ...
        # CSV
        str1='CSV/NewBurstFreq_OU_Trials%d_NoiseAmp%1.0f_%d.csv' \
             % (1, noiseamp, number)
        ...
        with open(str1, 'a', newline='') as csvfile:
            ...
            for esyn_i in np.arange(-75, -64, 1):    
                print('Esyn_i= ', esyn_i)
                    
                #%% Data processing
                #Check if file exists, and if so load it 
                
                str1='CSV_Spiking/SpikingData_OU_Esyni%1.0f_NoiseAmp%1.0f_%d.csv' \
