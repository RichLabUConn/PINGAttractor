...
Iext_e_max=.5
Iext_e_step=.025
numIext=len(np.arange(Iext_e_min, Iext_e_max+.01, Iext_e_step))

sync_means=np.zeros((repetitions,numIext))
sync_varOverTime=np.zeros((repetitions,numIext))
freq_means=np.zeros((repetitions,numIext))
freq_varOverTime=np.zeros((repetitions,numIext))
MFF_means=np.zeros((repetitions,numIext))
MFF_varOverTime=np.zeros((repetitions,numIext))
percent_means=np.zeros((repetitions,numIext))
percent_varOverTime=np.zeros((repetitions,numIext))
sync_onset_num=np.zeros((repetitions,numIext))
sync_offset_num=np.zeros((repetitions,numIext))
sync_duration=np.zeros((repetitions,numIext))
DCtoSpike_corr_e=np.zeros((repetitions,numIext))
DCtoSpike_corr_i=np.zeros((repetitions,numIext))
DCtoISImean_corr_e=np.zeros((repetitions,numIext))
DCtoISImean_corr_i=np.zeros((repetitions,numIext))
DCtoISIstd_corr_e=np.zeros((repetitions,numIext))
DCtoISIstd_corr_i=np.zeros((repetitions,numIext))
ISImeantoISIstd_corr_e=np.zeros((repetitions,numIext))
ISImeantoISIstd_corr_i=np.zeros((repetitions,numIext))
linreg_r=np.zeros((repetitions,numIext))
linreg_slope=np.zeros((repetitions,numIext))
syncstart=np.zeros((repetitions,numIext))
syncstop=np.zeros((repetitions,numIext))
syncduration=np.zeros((repetitions,numIext))

#%% Import and store data for repetitions

padcount=np.zeros(numIext)

for rep in np.arange(1, repetitions+1, 1):    
    
    # print(rep)
    str1='CSV/Stats_OUNewInitial_Trials1_NoiseAmp%d_%d.csv' \
         % (noiseamp, rep)
    ...
    # Pad with Nan's
    if np.around(stats_csv[-1][0], decimals=2)!=Iext_e_max:
        fill=np.arange(stats_csv[-1][0]+Iext_e_step,Iext_e_max+.01,Iext_e_step)
        for count, pad in enumerate(fill):
            temp=np.empty((1,25))
            temp[0,0]=pad
            temp[0,1:]=np.nan
            stats_csv=np.append(stats_csv, temp, axis=0)
            print('A repetition did not finish Iext_e=', pad)
            padcount[-(len(fill)-count)]=padcount[-(len(fill)-count)]+1
    ...
    syncstart[rep-1]=stats_csv[:,22]
    syncstop[rep-1]=stats_csv[:,23]
    syncduration[rep-1]=stats_csv[:,24]
...
syncstart_mean=np.nanmean(syncstart,axis=0)
syncstart_std=np.nanstd(syncstart,axis=0)
syncstop_mean=np.nanmean(syncstop,axis=0)
syncstop_std=np.nanstd(syncstop,axis=0)
syncduration_mean=np.nanmean(syncduration,axis=0)
syncduration_std=np.nanstd(syncduration,axis=0)





#%% Calculate percent of sims that synchronize
syncpercent=np.zeros(numIext)
temp_nonnan=~np.isnan(syncstart)
for i in range(numIext):
    syncpercent[i]=temp_nonnan[:,i].sum()/(repetitions-padcount[i])
...
with open(str1, 'w', newline='') as csvfile:
    ...
    for i in range(len(Iext_e)):
        mystr=[Iext_e[i], sync_means_mean[i], sync_means_std[i], sync_varOverTime_mean[i], sync_varOverTime_std[i],
               ...
               syncstart_mean[i], syncstart_std[i], syncstop_mean[i], syncstop_std[i], syncduration_mean[i], syncduration_std[i], syncpercent[i]]
...
ax1.errorbar(Iext_e, sync_means_mean, sync_means_std, fmt='o', markersize=8, capsize=10)
ax1.set_ylabel('Synchrony Measure')
ax1.set_ylim([0, 1])

ax2.errorbar(Iext_e, freq_means_mean, freq_means_std, fmt='o', markersize=8, capsize=10)
ax2.set_ylabel('Burst Frequency')
ax2.set_ylim([0, 150])

ax3.errorbar(Iext_e, MFF_means_mean, MFF_means_std, fmt='o', markersize=8, capsize=10)
ax3.set_ylabel('Spiking Frequency')
ax3.set_ylim([0, 100])

ax4.errorbar(Iext_e, percent_means_mean, percent_means_std, fmt='o', markersize=8, capsize=10)
ax4.set_ylabel('Spiking Percentage\n Per Burst')
ax4.set_xlabel(r'$I_{ext}$ ($\mu$ A)')
ax4.set_ylim([0, 1])
...
str2='AverageOverRepetitionsPlots/VaryIextE_WholeSim_Repetitions%d_NoiseAmp%d.png' \
    % (repetitions, noiseamp)
...
ax1.errorbar(Iext_e, sync_varOverTime_mean, sync_varOverTime_std, fmt='o', markersize=8, capsize=10)
ax1.set_ylabel('Synchrony Measure')

ax2.errorbar(Iext_e, freq_varOverTime_mean, freq_varOverTime_std, fmt='o', markersize=8, capsize=10)
ax2.set_ylabel('Burst Frequency')

ax3.errorbar(Iext_e, MFF_varOverTime_mean, MFF_varOverTime_std, fmt='o', markersize=8, capsize=10)
ax3.set_ylabel('Spiking Frequency')

ax4.errorbar(Iext_e, percent_varOverTime_mean, percent_varOverTime_std, fmt='o', markersize=8, capsize=10)
ax4.set_ylabel('Spiking Percentage\n Per Burst')
ax4.set_xlabel(r'$I_{ext}$ ($\mu$ A)')
...
str2='AverageOverRepetitionsPlots/VaryIextE_VarOverTime_Repetitions%d_NoiseAmp%d.png' \
    % (repetitions, noiseamp)
plt.savefig(str2, dpi=300, format='png', bbox_inches='tight')
...
ax1.errorbar(Iext_e, sync_onset_num_mean, sync_onset_num_std, fmt='o', markersize=8, capsize=10)
ax1.set_ylabel('Synchrony Onsets')

ax2.errorbar(Iext_e, sync_offset_num_mean, sync_offset_num_std, fmt='o', markersize=8, capsize=10)
ax2.set_ylabel('Synchrony Offsets')

ax3.errorbar(Iext_e, sync_duration_mean, sync_duration_std, fmt='o', markersize=8, capsize=10)
ax3.set_ylabel('Synchrony Duration')
ax3.set_xlabel(r'$I_{ext}$ ($\mu$ A)')
...
str2='AverageOverRepetitionsPlots/VaryIextE_SyncTransitions_Repetitions%d_NoiseAmp%d.png' \
    % (repetitions, noiseamp)
plt.savefig(str2, dpi=300, format='png', bbox_inches='tight')
...
ax1.errorbar(Iext_e, DCtoSpike_corr_e_mean, DCtoSpike_corr_e_std, fmt='o', markersize=8, capsize=10)
ax1.set_ylabel('Correlation, DC Input \n to Spiking Frequency')

ax2.errorbar(Iext_e, DCtoISImean_corr_e_mean, DCtoISImean_corr_e_std, fmt='o', markersize=8, capsize=10)
ax2.set_ylabel('Correlation, DC Input \n to Mean ISI')

ax3.errorbar(Iext_e, DCtoISIstd_corr_e_mean, DCtoISIstd_corr_e_std, fmt='o', markersize=8, capsize=10)
ax3.set_ylabel('Correlation, DC Input \n to SD ISI')

ax4.errorbar(Iext_e, ISImeantoISIstd_corr_e_mean, ISImeantoISIstd_corr_e_std, fmt='o', markersize=8, capsize=10)
...
str2='AverageOverRepetitionsPlots/VaryIextE_CorrelationsE_Repetitions%d_NoiseAmp%d.png' \
    % (repetitions, noiseamp)
plt.savefig(str2, dpi=300, format='png', bbox_inches='tight')
...
ax1.errorbar(Iext_e, DCtoSpike_corr_i_mean, DCtoSpike_corr_i_std, fmt='o', markersize=8, capsize=10)
ax1.set_ylabel('Correlation, DC Input \n to Spiking Frequency')

ax2.errorbar(Iext_e, DCtoISImean_corr_i_mean, DCtoISImean_corr_i_std, fmt='o', markersize=8, capsize=10)
ax2.set_ylabel('Correlation, DC Input \n to Mean ISI')

ax3.errorbar(Iext_e, DCtoISIstd_corr_i_mean, DCtoISIstd_corr_i_std, fmt='o', markersize=8, capsize=10)
ax3.set_ylabel('Correlation, DC Input \n to SD ISI')

ax4.errorbar(Iext_e, ISImeantoISIstd_corr_i_mean, ISImeantoISIstd_corr_i_std, fmt='o', markersize=8, capsize=10)
ax4.set_ylabel('Correlation, Mean ISI \n to SD ISI')
ax4.set_xlabel(r'$I_{ext}$ ($\mu$ A)')
...
str2='AverageOverRepetitionsPlots/VaryIextE_CorrelationsI_Repetitions%d_NoiseAmp%d.png' \
    % (repetitions, noiseamp)
plt.savefig(str2, dpi=300, format='png', bbox_inches='tight')
...
ax1.errorbar(Iext_e, linreg_r_mean, linreg_r_std, fmt='o', markersize=8, capsize=10)
ax1.set_ylabel('Linear Regression Rsquared')

ax2.errorbar(Iext_e, linreg_slope_mean, linreg_slope_std, fmt='o', markersize=8, capsize=10)
ax2.set_ylabel('Linear Regression Slope')

ax2.set_xlabel(r'$I_{ext}$ ($\mu$ A)')
...
str2='AverageOverRepetitionsPlots/VaryIextE_LinearRegression_Repetitions%d_NoiseAmp%d.png' \
    % (repetitions, noiseamp)
plt.savefig(str2, dpi=300, format='png', bbox_inches='tight')
...
ax1.errorbar(Iext_e, syncstart_mean, syncstart_std, fmt='o', markersize=8, capsize=10)
ax1.set_ylabel('Synchrony Start')

ax2.errorbar(Iext_e, syncstop_mean, syncstop_std, fmt='o', markersize=8, capsize=10)
ax2.set_ylabel('Synchrony Stop')

ax3.errorbar(Iext_e, syncduration_mean, syncduration_std, fmt='o', markersize=8, capsize=10)
ax3.set_ylabel('Synchrony Duration')

ax4.errorbar(Iext_e, syncpercent, 0, fmt='o', markersize=8, capsize=10)
ax4.set_ylabel('Percent of Sims \n that Synchronize')
ax4.set_ylim([0,1])


ax4.set_xlabel(r'$I_{ext}$ ($\mu$ A)')

titlestr='New Synchrony Duration; Noise Amplitude=%1.7f' \
    % (noiseamp/1000000)
fig.suptitle(titlestr)
...
str2='AverageOverRepetitionsPlots/VaryIextE_NewSyncDur_Repetitions%d_NoiseAmp%d.png' \
    % (repetitions, noiseamp)
plt.savefig(str2, dpi=300, format='png', bbox_inches='tight')
