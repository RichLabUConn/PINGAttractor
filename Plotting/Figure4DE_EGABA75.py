...
Iext_e_range=[25,28,30,33,35]
for noiseamp in [0, 2000]:
    nonPINGduration=np.zeros([len(Iext_e_range), 100])
    gammaEvents=np.zeros([len(Iext_e_range), 100])

    # CSV
    str1='FinalFigure4Quants/GammaEventRate_SyncPercent_EGABA75_NoiseAmp%1.0f.csv' % (noiseamp)
    ...
    with open(str1, 'a', newline='') as csvfile:
        ...
        for Iext_e_index, Iext_e in enumerate(Iext_e_range):
            ...
            for number_index, number in enumerate(np.arange(1,101,1)):
                ...
                # str2=r'D:\OneDrive - University of Connecticut\3) Papers\1) Steve Paper\1) Redoing simulations\3) New initial conditions\CSV_Spiking\SpikingData_OUNewInitial_NoiseAmp%1.0f_Iext%1.0f_%d.csv' \
                #      % (noiseamp, Iext_e, number)

                str2=r'/Users/sbr23005/Library/CloudStorage/OneDrive-UniversityofConnecticut/101) SBR_ResearchStorage - Documents/PostdocPapers/1) Steve Paper/1) Redoing simulations/3) New initial conditions/CSV_Spiking/SpikingData_OUNewInitial_NoiseAmp%1.0f_Iext%1.0f_%d.csv' \
                     % (noiseamp, Iext_e, number)
                     
                if os.path.exists(str2)==True:
                     
                    spiking_csv=np.genfromtxt(str2, delimiter=',')
        ...
        gammaEventRate=gammaEvents/nonPINGduration
        gammaEventRate_mean=np.mean(gammaEventRate,1)
        gammaEventRate_std=np.std(gammaEventRate,1)
        
        syncYN=nonPINGduration<18
        syncYN_mean=np.mean(syncYN,1)
        syncYN_std=np.std(syncYN,1)
        
        # gammaEvents_mean=np.mean(gammaEvents,1) 
        # gammaEvents_std=np.std(gammaEvents,1)
        # nonPINGduration_mean=np.mean(nonPINGduration,1)
        # nonPINGduration_std=np.std(nonPINGduration,1)
        
        for Iext_e_index, Iext_e in enumerate(Iext_e_range):           
            mystr=[Iext_e, gammaEventRate_mean[Iext_e_index], gammaEventRate_std[Iext_e_index], syncYN_mean[Iext_e_index], syncYN_std[Iext_e_index]]
