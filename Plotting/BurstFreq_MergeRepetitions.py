...
for rep in np.arange(1, repetitions+1, 1):    
    ...
    print(rep)
    str1='CSV/NewBurstFreq_OU_Trials1_NoiseAmp%d_%d.csv' \
         % (noiseamp, rep)
...
str1='AverageOverRepetitionsPlots/NewBurstFreq_VaryEsynI_Repetitions%d_NoiseAmp%d.csv' \
...
with open(str1, 'w', newline='') as csvfile:
    ...
    for i in range(len(esyn_i)):
        mystr=[esyn_i[i], freq_means_mean[i], freq_means_std[i]]
