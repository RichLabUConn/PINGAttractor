...
ax=sns.violinplot(x='EGABA', y='Duration', hue='NoiseAmp', data=df, ax=ax1, inner=None)
plt.scatter(x=[-.2, .2, .8, 1.2], y=duration_means, c='k')   
ax.set_xlabel('')
ax.set_xticklabels([])

ax.set_ylabel('Gamma duration (s)')
ax.spines[['right', 'top']].set_visible(False)
ax.legend(title=r'$SD_V$ (mV)', loc='right', bbox_to_anchor=(1.6, .5),ncol=1)
ax.legend_.texts[0].set_text('0.0')
ax.legend_.texts[1].set_text('1.087')
...
fig,ax1=plt.subplots()
fig.set_size_inches(2.5,2)
ax=sns.violinplot(x='EGABA', y='PeakFreq', hue='NoiseAmp', data=df, ax=ax1, inner=None)
plt.scatter(x=[-.2, .2, .8, 1.2], y=peakfreq_means, c='k')      
ax.set_xlabel(r'$E_{\text{GABA}}$ (mV)')
