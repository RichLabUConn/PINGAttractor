...
T=20000              #Length of simulation, ms
# T=4000
dt=.02              #Time step
...
Iext_e_mean=0.3          #Mean excitatory input
...
if len(sys.argv)==1:
    ...
    esyn_i_max=-55.0        # Max esyn_i for loop
    ...
    dt=0.02                  # Time step
    esyn_i_min=-75.0        # Min esyn_i for loop
    esyn_i_max=-75.0        # Max esyn_i for loop
    esyn_i_step=1.0         # esyn_i step for loop
    syncsample=int(1000/dt) # How often to calculate synchrony
...
def run_sim():
    ...
    #Initialize lists to hold measures
    sync_stdOverTime=[]
    sync_FullSim=[]
    freq_stdOverTime=[]
    freq_FullSim=[]
    MFF_stdOverTime=[]
    MFF_FullSim=[]
    meanSpikePercentagePerBurst_FullSim=[]
    meanSpikePercentagePerBurst_stdOverTime=[]
    

    
    #%%Trial loop
    for q in range(trials):
        print('Starting Trial', q, ' Iext_e_mean=', Iext_e_mean)
        ...
        Iext_e_initial=sample_inputs(numcells_e, -.05, hetero_e*2)
        ...
        for i in range(int(T/dt)):
            ...
            if i*dt<2000:
                Iapp_e=Iext_e_initial-np.multiply(Isynin_ee,(v_e[:,-2]-esyn_e))-np.multiply(Isynin_ie,(v_e[:,-2]-esyn_i))
            ...
            #Integrate
            v_e[:,-1], z_e[:,-1], h_e[:,-1], n_e[:,-1], ou_i_e=eulermaruyama(v_e[:,-2], z_e[:,-2], h_e[:,-2], n_e[:,-2], Iapp_e, D, ou_i_e)
            v_i[:,-1], z_i[:,-1], h_i[:,-1], n_i[:,-1], ou_i_i=eulermaruyama(v_i[:,-2], z_i[:,-2], h_i[:,-2], n_i[:,-2], Iapp_i, D, ou_i_i)
        ...
        #Calculate E Synchrony Measure over entire simulation        
        measure_start=2000
        measure_end=T
        # measure_end=2500
        duration_sec=(measure_end-measure_start)/1000
        spikes_e_end=processSpikesForSync(spikes_e, measure_start, measure_end, numcells_e)
        
        sync_total, freq_total, MFF_total, meansig_total, numpeaks_total, peaks_total=syncmeasure(numcells_e, spikes_e_end, 2, duration_sec)
        # if numpeaks_total>0:
        #     meanSpikePercentagePerBurst_total=(MFF_total*duration_sec)/numpeaks_total
        # else:
        #     meanSpikePercentagePerBurst_total=0
            
        sync_FullSim=sync_total
        freq_FullSim=freq_total
        MFF_FullSim=MFF_total
        # meanSpikePercentagePerBurst_FullSim=meanSpikePercentagePerBurst_total

                
        #Calculate E Synchrony Measure over time and calculate its std
        sync_overtime=[]
        freq_overtime=[]
        MFF_overtime=[]
        meanSpikePercentagePerBurst_overtime=[]
        
        window_coeff=10
        if freq_total>0:
            window=window_coeff*(1000/freq_total)
        else:
            window=1000
        if window>1000:
            window=1000
        windows=np.arange(measure_start,measure_end, window)

        for i in range(len(windows)-1):
            measure_start_temp=windows[i]
            measure_end_temp=windows[i+1]
            duration_sec=(measure_end_temp-measure_start_temp)/1000
            
            spikes_e_end=processSpikesForSync(spikes_e, measure_start_temp, measure_end_temp, numcells_e)
            sync_now, freq_now, MFF_now, meansig_now, numpeaks_now, peaks_now=syncmeasure(numcells_e, spikes_e_end, 2, duration_sec)
            if numpeaks_now>0 and sync_now>.2:
                meanSpikePercentagePerBurst_now=(MFF_now*duration_sec)/numpeaks_now
            else:
                meanSpikePercentagePerBurst_now=np.nan
            
            sync_overtime.append(sync_now)
            freq_overtime.append(freq_now)
            MFF_overtime.append(MFF_now)
            meanSpikePercentagePerBurst_overtime.append(meanSpikePercentagePerBurst_now)
        
        plot_times=windows[:-1]+(windows[1:]-windows[:-1])/2  
        
        sync_stdOverTime=np.nanstd(sync_overtime)
        freq_stdOverTime=np.nanstd(freq_overtime)
        MFF_stdOverTime=np.nanstd(MFF_overtime)
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", category=RuntimeWarning)
            meanSpikePercentagePerBurst_stdOverTime=np.nan_to_num(np.nanstd(meanSpikePercentagePerBurst_overtime))
            meanSpikePercentagePerBurst_FullSim=np.nan_to_num(np.nanmean(meanSpikePercentagePerBurst_overtime))
        

        # if np.nanstd(sync_overtime)>0.100:
            
        str1='SpikingData_OUNewInitial_NoiseAmp%1.0f_Iext%1.0f_%d.csv' \
             % (D*1000000, Iext_e_mean*100, number)
        ...
        # Calculating synchrony offset/onset via crossings at .2
        sync_onset=[]
        sync_offset=[]
        sync_checkcrossings=[x-.2 for x in sync_overtime]
        sync_crossingindicies=np.where(np.diff(np.sign(sync_checkcrossings)))[0]
        for x in sync_crossingindicies:
            if sync_checkcrossings[int(x)]<0:
                sync_onset.append(plot_times[int(x)])
            else:
                sync_offset.append(plot_times[int(x)])
        
        sync_onset_num=len(sync_onset)
        sync_offset_num=len(sync_offset)
        
        
        # Use above to approximate time spent in synchrony
        sync_duration=0
        if sync_onset_num==0 and sync_offset_num==0:
            if sync_FullSim>.2:
                sync_duration=T-measure_start
        elif sync_overtime[0]>.2:
            if sync_offset_num>sync_onset_num:
                sync_onset.insert(0,measure_start)
                for i in range(len(sync_offset)):
                    sync_duration=sync_duration+(sync_offset[i]-sync_onset[i])
            elif sync_offset_num==sync_onset_num:
                sync_onset.insert(0,measure_start)
                sync_offset.append(T)
                for i in range(len(sync_offset)):
                    sync_duration=sync_duration+(sync_offset[i]-sync_onset[i])
        elif sync_overtime[0]<.2:
            if sync_onset_num>sync_offset_num:
                sync_offset.append(T)
                for i in range(len(sync_offset)):
                    sync_duration=sync_duration+(sync_offset[i]-sync_onset[i])
            elif sync_onset_num==sync_offset_num:
                for i in range(len(sync_offset)):
                    sync_duration=sync_duration+(sync_offset[i]-sync_onset[i])
        ...
        #%% Single Cell Calculations and Plots
        DC_e=Iext_e
        DC_i=Iext_i
        hist_e=np.array(histogram_SpikeRateIndividualCells(spikes_e, 1000, T,numcells_e))
        hist_i=np.array(histogram_SpikeRateIndividualCells(spikes_i, 1000, T,numcells_i))
        DCtoSpikeRate_corr_e=np.corrcoef(DC_e,hist_e)
        DCtoSpikeRate_corr_i=np.corrcoef(DC_i,hist_i)
        
        ISI_mean_e, ISI_std_e=calculate_ISIs(spikes_e, 1000, T,numcells_e)
        ISI_mean_i, ISI_std_i=calculate_ISIs(spikes_i, 1000, T,numcells_i)
        DCtoISIMean_corr_e=np.corrcoef(DC_e,ISI_mean_e)
        DCtoISIMean_corr_i=np.corrcoef(DC_i,ISI_mean_i)
        DCtoISIstd_corr_e=np.corrcoef(DC_e,ISI_std_e)
        DCtoISIstd_corr_i=np.corrcoef(DC_i,ISI_std_i)
        ISImeantoISIstd_corr_e=np.corrcoef(ISI_mean_e,ISI_std_e)
        ISImeantoISIstd_corr_i=np.corrcoef(ISI_mean_i,ISI_std_i)
        
        
                   
                               
                    
        #%% Stats Over Time Plots
        plt.rcParams['font.size']=18
        fig, (ax1, ax2, ax3, ax4)=plt.subplots(4,1, sharex=True)
        fig.set_size_inches(24,17)
        
        ax1.plot(plot_times, sync_overtime, 'go-', ms=5)
        ax1.set_ylabel('Synchrony Measure', fontsize=16)
        ax1.set_ylim([0, 1])
        
        ax2.plot(plot_times, freq_overtime, 'go-', ms=5)
        ax2.set_ylabel('Burst Frequency', fontsize=16)
        ax2.set_ylim([0, 150])
    
        ax3.plot(plot_times, MFF_overtime, 'go-', ms=5)
        ax3.set_ylabel('Spiking Frequency', fontsize=16)
        ax3.set_ylim([0, 100])
    
        ax4.plot(plot_times, meanSpikePercentagePerBurst_overtime, 'go-', ms=5)
        ax4.set_ylabel('Spiking Percentage \n Per Burst', multialignment='center', fontsize=16)
        ax4.set_xlabel('Time (ms)')
        ax4.set_ylim([0, 1])
    
        titlestr='Iext_e_mean=%1.2f, Noise Amplitude=%1.6f' % (Iext_e_mean, D)
        fig.suptitle(titlestr)
          
        plt.tight_layout()
    
    
        str2='StatsOverTime_OUNewInitial_NoiseAmp%1.0f_Iext%1.0f_%d.png' \
             % (D*1000000, Iext_e_mean*100, number)
        ...
        #%% Firing Rate Histogram stuff
        names=dict([(0, 'shorter'), (1, 'short'), (2, 'mid'), (3, 'long')])
        
        vars()["window_"+names[0]]=2
        vars()["window_"+names[1]]=5
        vars()["window_"+names[2]]=15
        vars()["window_"+names[3]]=30
        scale=12.5
        ...
        for i in range(len(names)):
            ...
            vars()["peaks_"+names[i]], vars()["peak_heights_"+names[i]]= \
                find_peaks(vars()["firingrate_"+names[i]],height=1, prominence=.1, distance=75)
            ...
            vars()["legstr"+str(i+1)]='Gaussian SD: %d ms' % vars()["window_"+names[i]]
        ...
        print('Ending Trial', q, ' Iext_e_mean=', Iext_e_mean)
        
        
        
        
    return sync_FullSim, sync_stdOverTime, freq_FullSim, freq_stdOverTime, \
        MFF_FullSim, MFF_stdOverTime, meanSpikePercentagePerBurst_FullSim, meanSpikePercentagePerBurst_stdOverTime, \
            sync_onset_num, sync_offset_num, sync_duration,\
                DCtoSpikeRate_corr_e[0,1], DCtoSpikeRate_corr_i[0,1],\
                    DCtoISIMean_corr_e[0,1], DCtoISIMean_corr_i[0,1],\
                        DCtoISIstd_corr_e[0,1], DCtoISIstd_corr_i[0,1],\
                            ISImeantoISIstd_corr_e[0,1], ISImeantoISIstd_corr_i[0,1], \
                                linreg_r, linreg_slope, \
                                    syncstart, syncstop, syncduration
...
freq_FullSim_IExt=[]
MFF_FullSim_IExt=[]
meanSpikePercentagePerBurst_FullSim_IExt=[]
sync_stdOverTime_IExt=[]
freq_stdOverTime_IExt=[]
MFF_stdOverTime_IExt=[]
meanSpikePercentagePerBurst_stdOverTime_IExt=[]
sync_onset_num_IExt=[]
sync_offset_num_IExt=[]
sync_duration_IExt=[]
DCtoSpike_corr_e_IExt=[]
DCtoSpike_corr_i_IExt=[]
DCtoISImean_corr_e_IExt=[]
DCtoISImean_corr_i_IExt=[]
DCtoISIstd_corr_e_IExt=[]
DCtoISIstd_corr_i_IExt=[]
ISImeantoISIstd_corr_e_IExt=[]
ISImeantoISIstd_corr_i_IExt=[]
linreg_r_IExt=[]
linreg_slope_IExt=[]
syncstart_IExt=[]
syncstop_IExt=[]
...
# CSV
str1='Stats_OUNewInitial_Trials%d_NoiseAmp%1.0f_%d.csv' \
     % (trials, D*1000000, number)
     
with open(str1, 'a', newline='') as csvfile:
    outputwriter=csv.writer(csvfile)


    for Iext_e_mean in np.arange(.25, .51, .025):
        
        sync_FullSim, sync_stdOverTime, freq_FullSim, freq_stdOverTime, \
            MFF_FullSim, MFF_stdOverTime, meanSpikePercentagePerBurst_FullSim, meanSpikePercentagePerBurst_stdOverTime, \
                sync_onset_num, sync_offset_num, sync_duration, \
                    DCtoSpike_corr_e, DCtoSpike_corr_i,\
                        DCtoISImean_corr_e, DCtoISImean_corr_i,\
                            DCtoISIstd_corr_e, DCtoISIstd_corr_i,\
                                ISImeantoISIstd_corr_e, ISImeantoISIstd_corr_i, \
                                    linreg_r, linreg_slope, \
                                        syncstart, syncstop, syncduration=run_sim()
        
        sync_FullSim_IExt.append(sync_FullSim)
        freq_FullSim_IExt.append(freq_FullSim)
        MFF_FullSim_IExt.append(MFF_FullSim)
        meanSpikePercentagePerBurst_FullSim_IExt.append(meanSpikePercentagePerBurst_FullSim)
        sync_stdOverTime_IExt.append(sync_stdOverTime)
        freq_stdOverTime_IExt.append(freq_stdOverTime)
        MFF_stdOverTime_IExt.append(MFF_stdOverTime)
        meanSpikePercentagePerBurst_stdOverTime_IExt.append(meanSpikePercentagePerBurst_stdOverTime)
        sync_onset_num_IExt.append(sync_onset_num)
        sync_offset_num_IExt.append(sync_offset_num)
        sync_duration_IExt.append(sync_duration)
        DCtoSpike_corr_e_IExt.append(DCtoSpike_corr_e)
        DCtoSpike_corr_i_IExt.append(DCtoSpike_corr_i)
        DCtoISImean_corr_e_IExt.append(DCtoISImean_corr_e)
        DCtoISImean_corr_i_IExt.append(DCtoISImean_corr_i)
        DCtoISIstd_corr_e_IExt.append(DCtoISIstd_corr_e)
        DCtoISIstd_corr_i_IExt.append(DCtoISIstd_corr_i)
        ISImeantoISIstd_corr_e_IExt.append(ISImeantoISIstd_corr_e)
        ISImeantoISIstd_corr_i_IExt.append(ISImeantoISIstd_corr_i)
        linreg_r_IExt.append(linreg_r)
        linreg_slope_IExt.append(linreg_slope)
        syncstart_IExt.append(syncstart)
        syncstop_IExt.append(syncstop)
        syncduration_IExt.append(syncduration)


         
        mystr=[Iext_e_mean, sync_FullSim, sync_stdOverTime, freq_FullSim, freq_stdOverTime, 
               MFF_FullSim, MFF_stdOverTime, meanSpikePercentagePerBurst_FullSim, meanSpikePercentagePerBurst_stdOverTime,
               sync_onset_num, sync_offset_num, sync_duration, DCtoSpike_corr_e, DCtoSpike_corr_i,
               DCtoISImean_corr_e, DCtoISImean_corr_i, DCtoISIstd_corr_e, DCtoISIstd_corr_i,
               ISImeantoISIstd_corr_e, ISImeantoISIstd_corr_i, linreg_r, linreg_slope,
               syncstart, syncstop, syncduration]
